...
class BayesClassifier:
    ...
    def train(self) -> None:
        ...
        for index, filename in enumerate(files, 1): # type: ignore
            ...
            try:
                ...
                pass
            word = self.tokenize(text)
            fnName = filename.split('-')
            print(fnName)
            if int(fnName[1]) == 1:
                self.update_dict(word, self.neg_freqs)
            elif int(fnName[1]) == 5:
                self.update_dict(word, self.pos_freqs)
            else:
                pass
    ...
    def classify(self, text: str) -> str:
        ...
        total_freq_pos = 0
        total_freq_neg = 0
        p_files = self.load_dict(self.pos_filename)
        n_files = self.load_dict(self.neg_filename)
        for num in p_files.values():
            total_freq_pos += num

        for num in n_files.values():
            total_freq_neg += num
        ...
        for word in text:
            if word in p_files.keys():
                pos_prob += math.log((p_files[word] + 1)/ total_freq_pos)
            if word in n_files.keys():
                neg_prob += math.log((n_files[word] + 1)/ total_freq_neg)
        x = ""
...
if __name__ == "__main__":
    ...
    # b = BayesClassifier()
    # print(b.classify('I love python'))
    # print(b.classify('I hate python'))
    pass
